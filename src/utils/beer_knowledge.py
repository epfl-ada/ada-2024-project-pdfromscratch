...
def add_all_knowledge(df_user_beer_style_past_ratings: pd.DataFrame, df_current_beer_per_style: pd.DataFrame):
    # Get the list of columns with the count and max values for each beer style
    count_columns = [col for col in df_user_beer_style_past_ratings.columns if "count" in col]
    max_columns = [col for col in df_current_beer_per_style.columns if "max" in col]

    # Add the global and local knowledge
    df_user_beer_style_past_ratings = add_global_knowledge(df_current_beer_per_style, df_user_beer_style_past_ratings, count_columns)
    df_user_beer_style_past_ratings, df_local_knowledge = add_local_knowledge(df_current_beer_per_style, df_user_beer_style_past_ratings, max_columns, count_columns)

    # Create the knowledge dataframe
    df_knowledge = df_user_beer_style_past_ratings[["user_id","day","beer_id","global_knowledge","local_knowledge","style_tried"]]
    ...
    df_knowledge.loc[:,"local_knowledge"] = df_knowledge.loc[:,"local_knowledge"].replace(0,np.nan)

    return df_user_beer_style_past_ratings, df_knowledge, df_local_knowledge, max_columns
...
def add_experts(
    ...
    df_local_knowledge_quantile_expert = df_best_local_per_user.iloc[:, 1:-1].replace(0,np.nan).quantile(quantile_thresh)
...
def get_beer_required_expert(df_user_beer_style_past_ratings: pd.DataFrame, df_local_knowledge_quantile_expert: pd.DataFrame, max_columns: list):
    ...
    max_available_beer_per_day = df_user_beer_style_past_ratings[max_columns+["day"]].groupby("day").max().reset_index()
    # Rename the columns to use column-wise multiplication between dataframes
    max_available_beer_per_day.columns = ["day"] + df_local_knowledge_quantile_expert.index.to_list()
    ...
    df_beer_required_to_be_expert_long = max_available_beer_per_day.melt(id_vars='day', var_name='Beer Style', value_name='Beers to be expert')
    return df_beer_required_to_be_expert_long

def get_expert_count(df_user_beer_style_past_ratings: pd.DataFrame, df_users: pd.DataFrame):
    ...
    """
    expert_columns = [col for col in df_user_beer_style_past_ratings.columns if "expert" in col]

    # Get the users who were experts at some point and add their countries
    df_user_beer_style_past_ratings.loc[:,"isExpert"] = df_user_beer_style_past_ratings[expert_columns].sum(axis=1)
    df_expert_users = df_user_beer_style_past_ratings.loc[df_user_beer_style_past_ratings["isExpert"] >= 1,["user_id"] + expert_columns].groupby("user_id").max().reset_index()
    df_expert_users = df_expert_users.merge(df_users[["user_id","user_country_code","user_country"]], how="left", on="user_id")
...
def get_mean_expert_vs_non(df_ratings: pd.DataFrame, df_user_beer_style_past_ratings: pd.DataFrame, expert_columns: list, k: int = 100) -> pd.DataFrame:
    ...
    Returns:
        df_means_all_styles:
    """
    df_expert_ratings = df_user_beer_style_past_ratings[expert_columns+["day","user_id","beer_id"]].groupby(["user_id","day"]).max().reset_index().merge(df_ratings[["user_id","day","beer_global_style","rating"]], how="inner", on=["user_id","day"])
    df_means = []
    for style in expert_columns:
        beer_rated_by_experts = df_expert_ratings[(df_expert_ratings[style] == 1)]["beer_id"].unique()
        top_k_beers = df_ratings[(df_ratings["beer_global_style"] == style.split("_")[0]) & (df_ratings["beer_id"].isin(beer_rated_by_experts))][["rating","beer_id"]].groupby("beer_id").count().reset_index().sort_values(by="rating",ascending=False).head(k)["beer_id"].to_numpy()
        ...
        df_means.append(df_same_beers[["beer_global_style","rating",style]].rename(columns={style:"expert"}))
