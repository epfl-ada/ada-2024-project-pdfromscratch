...
def remove_low_alcohol(dataframe: pd.DataFrame, dataframe_past: pd.DataFrame) -> tuple[pd.DataFrame]:
    ...
    dataframe_past = dataframe_past.drop(columns=['user_past_ratings_Low Alcohol Beer_count', 'user_past_ratings_Low Alcohol Beer_average'], errors='ignore')
...
def shift_past_count(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe.loc[:,"user_beer_style_past_ratings_count"] = dataframe["user_beer_style_past_ratings_count"] - 1
    # Set the past average to NaN for the first rating as there is no previous rating
    dataframe.loc[dataframe["user_beer_style_past_ratings_count"] == 0,"user_beer_style_past_ratings_average"] = np.nan
    return dataframe
