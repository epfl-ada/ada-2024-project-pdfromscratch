...
def remove_before_month(dataframe: pd.DataFrame, dataframe_past: pd.DataFrame, month: str) -> pd.DataFrame:
    ...
    dataframe_past.drop(dataframe_past[dataframe_past["month"] < month].index, axis=0, inplace=True)
    # Drop the ratings given by after the threshold date by users who joined before the threshold
    dataframe_past = dataframe_past[dataframe_past["user_id"].isin(dataframe["user_id"].drop_duplicates())]
